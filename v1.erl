...
do(FileName) ->
  {#xmlElement{content=[#xmlElement{content=Head}, Body | _]}, _Misc} = xmerl_scan:file(FileName),
  
  FN = hd( string:split(FileName, ".", trailing) ),
  write2new(FN, lists:reverse( show(Body, false, false, 
...
show(#xmlElement{name=_Name, content=C}, S, _, RA) -> child(C, S, RA);
...
pt(_, #xmlElement{name=a}, [#xmlText{value=V} | _]) ->
  V2 = string:trim(V),
  if V2 =/= "" , hd(V2) == 46 -> true; true -> false end;
pt(_, _, _) -> false.
